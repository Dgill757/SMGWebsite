...
class PremiumDemoGeneratorTests(unittest.TestCase):
    def test_vercel_returns_public_production_domain_not_protected_deployment_url(self):
        ...
        response.content = b'{}'
        response.json = lambda: {"id": "dpl_test", "url": "summit-demo-acme-randomhash.vercel.app"}
        project_response = AsyncMock()
        project_response.status_code = 200
        project_response.json = lambda: {"targets": {"production": {"alias": ["summit-demo-acme.vercel.app"]}}}
        public_response = AsyncMock()
        public_response.status_code = 200
        client = AsyncMock()
        client.__aenter__.return_value.post.return_value = response
        client.__aenter__.return_value.get.side_effect = [project_response, public_response]
